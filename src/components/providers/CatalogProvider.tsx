...
export function CatalogProvider({ children }: { children: React.ReactNode }) {
  ...
  const allProducts: CatalogProduct[] = useMemo(() => {
    const flattened = mergedCatalog.flatMap((cat) =>
      cat.subcategories.flatMap((sub) =>
        sub.products.map((product) => ({
          ...
          source: (product as CatalogProduct).source || "static",
        }))
      )
    )
    ...
    for (const product of flattened) {
      const existing = deduped.get(product.id)
  ...
  const filterByCategory = (product: CatalogProduct, categoryId: string) =>
    product.category === categoryId || product.categoryId === categoryId

  const filterBySubcategory = (product: CatalogProduct, categoryId: string, subcategoryId: string) =>
    filterByCategory(product, categoryId) &&
    (product.subcategory === subcategoryId || product.subcategoryId === subcategoryId)

  const getProductsByCategory = (categoryId: string) =>
    allProducts.filter((product) => {
      if (!filterByCategory(product, categoryId)) return false
      return true
    })

  const getProductsBySubcategory = (categoryId: string, subcategoryId: string) =>
    allProducts.filter((product) => {
      if (!filterBySubcategory(product, categoryId, subcategoryId)) return false
      return true
    })
