...
export default function OrdersPage() {
  ...
  const exportGstCsv = async () => {
    ...
    try {
      ...
      link.href = csvUrl
      link.download = `taitil-gst-orders-${new Date().toISOString().slice(0,10)}.csv`
      link.click()
      setTimeout(() => URL.revokeObjectURL(csvUrl), 1000)
      setExportStatus(rowCount ? `Exported ${rowCount} lines.` : 'Export is empty. Clear date filters or check payment status.')
    } catch (err) {
              ...
              <div className="divide-y divide-gray-100 text-sm">
                {carts.map((cart) => (
                  <div key={cart.id} className="py-3 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
                    ...
                    <div className="text-xs text-gray-600">Subtotal: {inrSymbol} {Math.round(cart.subtotalCents / 100).toLocaleString('en-IN')}</div>
                    <div className="flex flex-wrap gap-2">
                      {cart.items.slice(0, 3).map((item: any) => (
                        <span key={item.id} className="rounded-full border border-gray-200 px-2 py-1 text-xs">{item.product?.name} x {item.quantity}</span>
                      ))}
