...
const getConfig = (): DelhiveryConfig => {
  ...
  return {
    ...
    fetchEndpoint: process.env.DELHIVERY_FETCH_ENDPOINT || '/waybill/api/fetch/json/',
...
export const fetchWaybills = async (count = 1) => {
  const config = getConfig()
  const useSingleFetch = process.env.DELHIVERY_USE_FETCH_SINGLE === 'true'

  if (!config.clientName || useSingleFetch) {
    const singleResponse = await delhiveryRequest<unknown>(config.fetchEndpoint, { params: {} })
    let singleWaybill = ''
    if (typeof singleResponse === 'string') {
      singleWaybill = singleResponse.trim()
    } else if (singleResponse && typeof singleResponse === 'object' && 'waybill' in singleResponse) {
      singleWaybill = String((singleResponse as { waybill?: string }).waybill || '').trim()
    }
    if (!singleWaybill) {
      throw new Error(`Delhivery did not return a waybill. Response: ${JSON.stringify(singleResponse)}`)
    }
    return [singleWaybill]
  }
  ...
  const responseText = typeof (response as unknown) === 'string' ? (response as unknown as string).trim() : ''
  if (responseText) {
    return [responseText]
  }
  if (!Array.isArray(waybills) || waybills.length === 0) {
    throw new Error(`Delhivery did not return a waybill. Response: ${JSON.stringify(response)}`)
  }
  return waybills
}
...
export const createDelhiveryShipment = async (shipment: ShipmentInput) => {
  ...
  const config = getConfig()
  const orderDate = shipment.orderDate || new Date().toISOString()
  const payload = {
    pickup_location: {
      name: pickup.name,
      add: pickup.address,
    ...
    shipments: [
      {
        ...
        shipment_length: shipment.lengthCm,
        shipment_width: shipment.widthCm,
        shipment_height: shipment.heightCm,
...
export const fetchDelhiveryLabel = async (waybill: string): Promise<DelhiveryLabel> => {
  const config = getConfig()
  const url = buildUrl(config.baseUrl, config.labelEndpoint, { wbns: waybill, format: 'pdf' })
  const res = await fetch(url, {
  ...
  if (contentType.includes('application/json')) {
    ...
    const found = payload?.packages_found || payload?.packagesFound || 0
    if (!found) {
      throw new Error('Delhivery label not ready yet. Please wait a few minutes after shipment creation.')
    }
    throw new Error(`Delhivery label error: ${JSON.stringify(payload)}`)
  }
